...
func begin_interframe() -> void:
	...
	if not data.has('start_time'):
		data['start_time'] = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system())

func end_interframe() -> void:
	if not data.has('frame_type'):
		data['frame_type'] = FrameType.INTERFRAME
	if not data.has('start_time'):
		data['start_time'] = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system()) - 1
	data['end_time'] = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system())
	write_current_data()
