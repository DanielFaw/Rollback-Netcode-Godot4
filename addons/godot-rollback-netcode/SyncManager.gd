...
# This stuff is gross in godot 4, I'm sorry
var input_tick: int = 0: 
	get: return _input_tick
	set(value): _set_readonly_variable(value)
...
func _on_ping_timer_timeout() -> void:
	if peers.size() == 0:
		return
	var msg = {
		local_time = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system()),
	}
	for peer_id in peers:
...
func _on_received_ping(peer_id: int, msg: Dictionary) -> void:
	...
	msg['remote_time'] = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system())
	network_adaptor.send_ping_back(peer_id, msg)

func _on_received_ping_back(peer_id: int, msg: Dictionary) -> void:
	var system_time = Time.get_unix_time_from_datetime_string(Time.get_time_string_from_system())
...
func start_logging(log_file_path: String, match_info: Dictionary = {}) -> void:
	...
	#TODO: Check and update this for 4.0 as it no longer exists
...
func _reset() -> void:
	_input_tick = 0
	_current_tick = input_tick - input_delay
	_skip_ticks = 0
	_rollback_ticks = 0
	input_buffer.clear()
	state_buffer.clear()
...
func _physics_process(_delta: float) -> void:
	...
	if mechanized:
		...
		if debug_rollback_ticks > 0 and current_tick >= debug_rollback_ticks:
			_rollback_ticks = max(rollback_ticks, debug_rollback_ticks)
...
func ensure_current_tick_input_complete() -> bool:
	...
	if requested_input_complete_tick == 0 or requested_input_complete_tick > current_tick:
		_requested_input_complete_tick = current_tick
	return false
